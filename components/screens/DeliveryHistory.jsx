...
const MOCK_BOOKINGS = [
  {
    id: '1',
    bookingId: 'A4G-BUIN8-IAS09855',
    passengerId: '2022-15482324253',
    passengerName: 'Naiza F. Albina',
    fare: '₱ 185',
    fromLocation: 'SM CITY North EDSA Main Entrance',
    toLocation: '76 P Florentino Street',
  },
  {
    id: '2',
    bookingId: 'A5X-JK98K-QWE09233',
    passengerId: '2023-12345678901',
    passengerName: 'Miguel S. Cruz',
    fare: '₱ 210',
    fromLocation: 'Ayala Center Cebu',
    toLocation: 'Mactan Airport',
  },
  {
    id: '3',
    bookingId: 'A5X-JK98K-QWE09233',
      ...
      <Animated.View style={[styles.stickyHeader, { backgroundColor: colors.elevation.level1 }]}>
        ...
        <Card style={[styles.dateCard, { backgroundColor: colors.elevation.level1 }]}>
          <Card.Content style={styles.dateContent}>
            <Text variant="titleSmall" style={[styles.dateText, { color: colors.onSurface }]}>{currentTime}</Text>
          </Card.Content>
        </Card>
        ...
        <View style={styles.sortOptionsContainer}>
          ...
          <Menu visible={menuVisible} onDismiss={() => setMenuVisible(false)} anchor={<Button mode="outlined" onPress={() => setMenuVisible(true)}>{sortCriterion === 'none' ? 'Sort by' : sortCriterion}</Button>}>
            {[{ criterion: 'bookingId', title: 'Booking ID' }, { criterion: 'passengerName', title: 'Passenger Name' }, { criterion: 'fare', title: 'Fare' }].map(option => (
  ...
  return (
    <Card style={[styles.bookingCard, { backgroundColor: colors.surface }]}>
      <Card.Content>
        ...
        <Button mode="contained" onPress={() => console.log('Show Details')} style={[styles.checkLocationButton, { backgroundColor: colors.primary }]}>
            SHOW DETAILS
        </Button>
...
const styles = StyleSheet.create({
  ...
  checkLocationButton: { borderRadius: 25, marginTop: 5, alignSelf: 'center', width: '80%', elevation: 3, marginBottom: 5 },
