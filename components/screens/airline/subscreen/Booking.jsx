...
/**
 * LocationAutofill - Component for address dropdowns
 */
...
const Booking = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const { drop_off_location, drop_off_location_geo, address_details, delivery_fee, pricing_status } = route.params.locationData
      ...
      if (match) {
        ...
        // Use the delivery fee from SelectLocation if available
        if (delivery_fee !== undefined && pricing_status) {
          setDeliveryFee(delivery_fee)
          if (pricing_status === 'no_pricing') {
            showSnackbar('No pricing data available for the selected location')
          } else if (pricing_status === 'no_match') {
            showSnackbar('The selected location is either invalid or out of delivery bounds')
          }
        } else {
          // If no pricing info provided, set to 0 (user should use SelectLocation screen)
          setDeliveryFee(0)
        }
      }
    }, [route.params])
