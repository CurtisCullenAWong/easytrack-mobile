...
const AirlineNavigator = ({ navigation }) => {
  ...
  // Development-only warning if a screen is missing in Drawer.Navigator
  useEffect(() => {
      ...
      SECTIONS.forEach(section => {
        section.items.forEach(item => {
          if (item.screen && !registeredScreens.has(item.screen)) {
            console.warn(
              `[AirlineNavigator] Screen "${item.screen}" in section "${section.title}" is not registered.`
            )
          }
        })
      })
  ...
  // Android back button → trigger logout
  useEffect(() => {
  ...
  const handleThemeButton = () => toggleTheme()
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <Surface style={[styles.surface, { backgroundColor: colors.background }]}>
        ...
        <Text
          style={[
            styles.appName,
            { color: colors.primary, ...fonts.headlineLarge },
          ]}
        >
          EasyTrack
        </Text>
      ...
      {!isVerified && (
        <Surface
          style={[styles.warningSurface, { backgroundColor: colors.errorContainer }]}
          elevation={1}
        >
          <Text
            style={[
              styles.warningText,
              { color: colors.onErrorContainer, ...fonts.bodyMedium },
            ]}
          >
            NOTICE: Your account is not verified yet. Some features are limited.
            Please complete your profile verification to access all features. ⚠️
          </Text>
        </Surface>
      )}
...
/* --- ExpandableSection Subcomponent --- */
